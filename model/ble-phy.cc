...
#include "/home/mihir/IoT_HOLA_Project/ns3/ns-3-dev-master/src/ble/model/ble-radio-energy-model.h"
...
namespace ns3 {
	...
	BlePhy::BlePhy () : m_BleRadioEnergyModel (0)
	{
	...
	BlePhy::~BlePhy ()
	{
		...
		m_antenna = 0;
		m_txPsd = 0;
		m_BleRadioEnergyModel = 0;
	}

	void
	BlePhy::RegisterListener (BlePhyListener *listener)
	{
  		m_listeners.push_back(listener);
	}

	void
BlePhy::NotifyTxStart (Time duration, double txPowerDbm)
{
  NS_LOG_FUNCTION (this);
...
void
BlePhy::NotifyRxStart (Time duration)
{
  NS_LOG_FUNCTION (this);
  for (Listeners::const_iterator i = m_listeners.begin (); i != m_listeners.end (); i++)
    {
...
void
BlePhy::NotifyMaybeCcaBusyStart (Time duration)
{
  NS_LOG_FUNCTION (this);
  for (Listeners::const_iterator i = m_listeners.begin (); i != m_listeners.end (); i++)
    {
...
void
BlePhy::NotifySwitchingStart (Time duration)
{
  NS_LOG_FUNCTION (this);
  for (Listeners::const_iterator i = m_listeners.begin (); i != m_listeners.end (); i++)
    {
...
BlePhy::NotifyOn (void)
{
  ...
  for (Listeners::const_iterator i = m_listeners.begin (); i != m_listeners.end (); i++)
	...
	void BlePhy::ResumeFromOff(void)
	{
		return;
	}

	void BlePhy::SetOffMode(void)
	{
		return;
	}

	void
	BlePhy::SetBleRadioEnergyModel (const Ptr<BleRadioEnergyModel> BleRadioEnergyModel)
	{
  		m_BleRadioEnergyModel = BleRadioEnergyModel;
	}
		...
		BlePhy::StartTx (Ptr<Packet> packet)
		{
			...
			if(this->GetState() == BlePhy::State::TX)
			{
              this->ChangeState(BlePhy::State::TX_BUSY);
				...
				NotifyTxStart(txParams->duration, m_power);
				return true;
			}
			return false;  
		}
